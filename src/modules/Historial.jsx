...
const MODULES = ["Mantenimiento", "Preventivo", "Materiales", "Producción", "Calidad", "Aprobaciones"];
...
export default function Historial() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("[Historial] Error cargando 'activity_log':", err?.code, err?.message, err);
  ...
  if (loadError) {
    return (
      <CenteredMessage
        text={`No se pudo cargar el historial (${loadError.code || "error"}). Revisa que las reglas de Firestore estén publicadas y que tu cuenta esté aprobada.`}
      />
    );
  }
