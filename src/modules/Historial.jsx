...
export default function Historial() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => setLoadError(`No se pudo cargar el historial (${err.code || err.message}). Revisa que hayas publicado el firestore.rules más reciente.`)
    );
    return unsub;
  }, []);
  ...
  if (!entries) return <CenteredMessage text={loadError || "Cargando historial…"} />;
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        {(dateFrom || dateTo) && entries && entries.length >= 200 && (
          <p style={{ color: COLORS.safety, fontSize: 12, margin: "6px 0 0" }}>
            Ojo: el filtro de fechas solo busca dentro de esas últimas 200 — si buscas una fecha antigua y no aparece nada, puede que ya haya quedado fuera de ese límite.
          </p>
        )}
      </div>
