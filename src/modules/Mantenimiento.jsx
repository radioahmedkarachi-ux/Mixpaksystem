    ...
    <div>
          ...
          {columns.map((col) => (
            <div key={col.value}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {col.items.map((t) => (
                  <TaskCard key={t.id} task={t} onStatusChange={(s) => updateStatus(t, s)} onDelete={() => setConfirmDelete(t)} onOpen={() => setDetailTaskId(t.id)} />
                ))}
      ...
      {detailTask && (
        <DetailModal
          ...
          onEdit={() => { setEditTask(detailTask); setDetailTaskId(null); setModalOpen(true); }}
        />
      )}
      {confirmDelete && (
        <ConfirmDialog
