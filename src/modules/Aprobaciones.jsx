...
export default function Aprobaciones({ user }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (err) => setLoadError(`No se pudo cargar la lista de personas (${err.code || err.message}). Revisa que hayas publicado el firestore.rules más reciente.`)
    );
    return unsub;
  }, []);
  ...
  async function approve(person, aprobado) {
    try {
      ...
      alert("No se pudo actualizar la aprobación. Revisa tu conexión e inténtalo de nuevo.");
    }
  }
  ...
  async function changeRole(person, role) {
    try {
      ...
      alert("No se pudo cambiar la categoría. Revisa tu conexión e inténtalo de nuevo.");
    }
  }
  ...
  async function removePerson(person) {
    try {
      ...
      alert("No se pudo eliminar la cuenta. Revisa tu conexión e inténtalo de nuevo.");
    }
  }
  ...
  if (!people) return <CenteredMessage text={loadError || "Cargando personas…"} />;
...
function PersonRow({ person, approved, onApprove, onRoleChange, isSelf, onRemove }) {
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${approved ? COLORS.green : COLORS.safety}`, padding: "12px 14px", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, justifyContent: "space-between" }}>
      <div>
        <div style={{ fontWeight: 600, fontSize: 14 }}>{person.name ? `${person.name} — ${person.email}` : person.email}{isSelf && " (tú)"}</div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <select
          ...
          disabled={isSelf}
          title={isSelf ? "No puedes cambiar tu propia categoría desde aquí (evita que te bloquees a ti mismo). Usa 'Cambiar categoría' en la cabecera si quieres cambiarla." : ""}
          style={{ ...selectStyle, fontSize: 12, padding: "6px 8px", opacity: isSelf ? 0.5 : 1 }}
        >
