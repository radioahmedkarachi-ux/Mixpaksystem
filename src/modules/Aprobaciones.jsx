...
import { COLORS, selectStyle, ghostButtonStyle, CenteredMessage, ConfirmDialog, StatCard, EmptyState, logActivity, useToast } from "../shared.jsx";
...
export default function Aprobaciones({ user }) {
  ...
  async function approve(person, aprobado) {
    try {
      ...
      toast(aprobado ? "Cuenta aprobada." : "Aprobación revocada.");
    } catch (err) {
  ...
  async function changeRole(person, role) {
    try {
      ...
      toast("No se pudo cambiar la categoría.", "error");
    }
  }
