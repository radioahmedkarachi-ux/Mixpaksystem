...
import { Factory, Plus, Trash2, PlayCircle, PauseCircle, CheckCircle2, TrendingUp, Search, Download, Share2, Camera, FileText } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Produccion({ user }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => setLoadError(`No se pudo cargar Producción (${err.code || err.message}). Revisa que hayas publicado el firestore.rules más reciente.`)
    );
    return unsub;
  }, []);
  ...
  async function removeOrder(order) {
    try {
      ...
      alert("No se pudo eliminar la orden. Revisa tu conexión e inténtalo de nuevo.");
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18, alignItems: "center" }}>
        ...
        <button
          onClick={() => exportToPdf({
            filename: "ordenes-produccion",
            title: "Órdenes de producción",
            subtitle: `${filtered.length} orden(es)`,
            userEmail: user.email,
            rows: filtered.map((o) => ({
              Producto: o.product, Cliente: o.client || "", Lote: o.lot || "", Línea: o.line || "",
              Turno: SHIFTS.find((s) => s.value === o.shift)?.label || o.shift,
              Objetivo: o.targetQty, Producido: o.producedQty, Rechazado: o.scrapQty,
              Estado: statusMeta(o.status).label, Fecha: o.date,
            })),
          })}
          style={ghostButtonStyle}
        >
      ...
      {loading ? (
        <CenteredMessage text={loadError || "Cargando órdenes de producción…"} />
      ) : orders.length === 0 ? (
...
function OrderCard({ order, onStatusChange, onEdit, onDelete }) {
  ...
  async function share() {
    ...
    const result = await shareText(order.product, lines);
  ...
  async function submit(e) {
    ...
    if (files.length) {
      ...
      try {
        ...
        setSaving(false);
        setUploadStatus("");
        if (uploaded.length > (order?.photos || []).length) {
          await updateDoc(doc(db, "production_orders", docId), { photos: uploaded }).catch(() => {});
        }
        setError("La orden se guardó, pero no todas las fotos se pudieron subir a Cloudinary (las que sí subieron se guardaron).");
        return;
      }
    }
    setSaving(false);
