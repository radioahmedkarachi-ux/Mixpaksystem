...
import { Boxes, Plus, Trash2, AlertTriangle, Minus, PackagePlus, Search, Download, Share2, Camera, Image as ImageIcon, ArrowLeftRight, History, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Materiales({ user }) {
  ...
  async function adjustStock(item, delta) {
    ...
    try {
      ...
      toast("No se pudo ajustar el stock.", "error");
    }
  }

  async function registerMovement(item, { type, quantity, reason, lot }) {
    ...
    logActivity(user.email, "Materiales", type === "entrada" ? "Entrada de stock" : "Salida de stock", `${item.name}: ${quantity} ${item.unit} (${reason || "sin motivo"})`);
  }
...
function MaterialCard({ item, onAdjust, onEdit, onDelete, onMovement, onKardex }) {
  ...
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${expired || low ? COLORS.critical : expiringSoon ? COLORS.safety : COLORS.green}`, padding: "12px 14px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
        <button onClick={onMovement} style={{ ...ghostButtonStyle, padding: "6px 10px", flex: 1, justifyContent: "center", fontSize: 12 }}>
          <ArrowLeftRight size={13} /> Movimiento
        </button>
        <button onClick={onKardex} style={{ ...ghostButtonStyle, padding: "6px 10px", flex: 1, justifyContent: "center", fontSize: 12 }}>
...
function MaterialModal({ item, user, onClose }) {
  ...
  async function submit(e) {
    ...
    setSaving(false);
    setUploadStatus("");
    toast(item ? "Material actualizado." : "Material guardado.");
    onClose();
  }
...
function MovementModal({ item, onClose, onSubmit }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await onSubmit({ type, quantity: qty, reason, lot });
  ...
  return (
    <ModalShell onClose={onClose} title={`Movimiento · ${item.name}`}>
      ...
      <form onSubmit={submit}>
        ...
        <Field label="Lote (opcional, actualiza el lote del material)">
          <input value={lot} onChange={(e) => setLot(e.target.value)} style={inputStyle} placeholder={item.lot || "Ej. L-2026-0731"} />
        </Field>
        {error && <p style={{ color: COLORS.critical, fontSize: 13, marginBottom: 10 }}>{error}</p>}
        <button type="submit" disabled={saving} style={{ ...primaryButtonStyle, width: "100%", justifyContent: "center" }}>
...
function KardexModal({ item, onClose }) {
  ...
  return (
    <ModalShell onClose={onClose} title={`Kardex · ${item.name}`}>
      ...
      {moves === null ? (
        <p style={{ fontSize: 13, color: COLORS.textMuted }}>Cargando movimientos…</p>
      ) : moves.length === 0 ? (
