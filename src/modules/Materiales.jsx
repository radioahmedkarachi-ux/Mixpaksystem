...
export default function Materiales({ user }) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "material_lots"), (snap) => {
  ...
  async function registerMovement(item, { type, quantity, reason, lot, expiryDate, lotId }) {
    ...
    if (type === "entrada") {
      ...
      // Salida: descuenta de un lote concreto elegido en el formulario
      if (lotId) {
        const lotRef = doc(db, "material_lots", lotId);
        const lotSnap = await getDocs(query(collection(db, "material_lots"), where("materialId", "==", item.id)));
        const lotDoc = lotSnap.docs.find((d) => d.id === lotId);
        if (lotDoc) {
          ...
          <Field label="Lote de origen (sugerido: el que caduca antes)">
            {availableLots === null ? (
              ...
              <p style={{ fontSize: 13, color: COLORS.critical }}>
                Este material no tiene lotes registrados todavía (usa "Entrada" para crear uno, o el ajuste rápido +/-1 no distingue lote).
              </p>
            ) : (
...
function LotsModal({ item, onClose }) {
  ...
  return (
    <ModalShell onClose={onClose} title={`Lotes · ${item.name}`}>
      ...
      {lots === null ? (
        <p style={{ fontSize: 13, color: COLORS.textMuted }}>Cargando lotes…</p>
      ) : active.length === 0 ? (
