...
import { Boxes, Plus, Trash2, AlertTriangle, Minus, PackagePlus, Search, Download, Share2, Camera, Image as ImageIcon, ArrowLeftRight, History, ArrowDownCircle, ArrowUpCircle, FileText, QrCode, ScanLine } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Materiales({ user }) {
  ...
  async function handleScan() {
    ...
    if (!value) return;
    const id = value.startsWith("material:") ? value.slice("material:".length) : value;
    const found = items.find((it) => it.id === id);
    if (found) {
      setEditItem(found);
      setModalOpen(true);
    } else {
      alert("Ese código QR no corresponde a ningún material de esta app.");
    }
  }
  ...
  return (
    <div>
      ...
      {qrItem && (
        <QrModal
          title={`QR · ${qrItem.name}`}
          label={qrItem.name}
          value={`material:${qrItem.id}`}
...
function MaterialCard({ item, expiryDate, onAdjust, onEdit, onDelete, onMovement, onKardex, onLots, onQr }) {
  ...
  return (
    <div style={{ background: COLORS.panel, borderLeft: `5px solid ${expired || low ? COLORS.critical : expiringSoon ? COLORS.safety : COLORS.green}`, padding: "12px 14px" }}>
      ...
      <button onClick={onQr} style={{ ...ghostButtonStyle, padding: "6px 8px", width: "100%", justifyContent: "center", fontSize: 12, marginTop: 6 }}>
        <QrCode size={13} /> Ver código QR
      </button>
