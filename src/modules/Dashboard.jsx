...
export default function Dashboard({ user, goTo, role }) {
  ...
  useEffect(() => {
    const unsubs = [
      ...
      onSnapshot(collection(db, "materials"), (snap) => setMaterials(snap.docs.map((d) => ({ id: d.id, ...d.data() })))),
      onSnapshot(collection(db, "material_lots"), (snap) => setMaterialLots(snap.docs.map((d) => d.data()))),
