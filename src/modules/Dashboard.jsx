...
export default function Dashboard({ user, goTo, role }) {
  ...
  useEffect(() => {
    const onErr = (source) => (err) => {
      console.error(`[Dashboard] Error cargando '${source}':`, err?.code, err?.message, err);
      setLoadError({ source, code: err?.code, message: err?.message });
    };
    const unsubs = [
      onSnapshot(collection(db, "tasks"), (snap) => setTasks(snap.docs.map((d) => d.data())), onErr("tasks")),
      onSnapshot(collection(db, "preventive_plans"), (snap) => setPlans(snap.docs.map((d) => d.data())), onErr("preventive_plans")),
      onSnapshot(collection(db, "materials"), (snap) => setMaterials(snap.docs.map((d) => d.data())), onErr("materials")),
      onSnapshot(collection(db, "production_orders"), (snap) => setOrders(snap.docs.map((d) => d.data())), onErr("production_orders")),
      onSnapshot(collection(db, "quality_issues"), (snap) => setIssues(snap.docs.map((d) => d.data())), onErr("quality_issues")),
    ];
    return () => unsubs.forEach((u) => u());
  }, []);

  const loading = !tasks || !plans || !materials || !orders || !issues;
  ...
  const stats = useMemo(() => {
    ...
    const vencidosPreventivo = activePlans.filter((p) => p.dueDate && p.dueDate < today).length;
    const semanaPreventivo = activePlans.filter((p) => {
      if (!p.dueDate) return false;
      const days = Math.round((new Date(p.dueDate) - new Date(today)) / 86400000);
    ...
    return { pendientesMtto, criticasMtto, vencidasMtto, vencidosPreventivo, semanaPreventivo, bajoMinimo, caducando, producedToday, eficiencia, abiertasCalidad, criticasCalidad };
  }, [tasks, plans, materials, orders, issues, loading]);
  ...
  if (loadError) {
    return (
      <CenteredMessage
        text={`No se pudo cargar '${loadError.source}' (${loadError.code || "error"}). Revisa que las reglas de Firestore estén publicadas y que tu cuenta esté aprobada.`}
      />
    );
  }
  ...
  return (
    <div>
      ...
      <Section title="Preventivo" icon={CalendarClock} onClick={() => goTo("preventivo")} visible={allowed.includes("preventivo")}>
        <StatCard label="Vencidos" value={stats.vencidosPreventivo} color={COLORS.critical} Icon={AlertTriangle} />
        <StatCard label="Esta semana" value={stats.semanaPreventivo} color={COLORS.safety} Icon={CalendarDays} />
      </Section>
