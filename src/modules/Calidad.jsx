...
import { ShieldCheck, Plus, Trash2, AlertOctagon, Camera, Image as ImageIcon, Search, Download, Share2, Pencil, FileText, PenLine } from "lucide-react";
import {
  COLORS,
  inputStyle,
  selectStyle,
...
export default function Calidad({ user }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => setLoadError(`No se pudo cargar Calidad (${err.code || err.message}). Revisa que hayas publicado el firestore.rules más reciente.`)
    );
    return unsub;
  }, []);
  ...
  async function removeIssue(issue) {
    try {
      ...
      alert("No se pudo eliminar la incidencia. Revisa tu conexión e inténtalo de nuevo.");
    }
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={() => { setEditIssue(null); setModalOpen(true); }} style={primaryButtonStyle}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18, alignItems: "center" }}>
        ...
        <button
          onClick={() => exportToPdf({
            filename: "incidencias-calidad",
            title: "Incidencias de calidad",
            subtitle: `${filtered.length} incidencia(s)`,
            userEmail: user.email,
            rows: filtered.map((i) => ({
              Título: i.title, Tipo: TYPES.find((t) => t.value === i.type)?.label || i.type,
              Severidad: i.severity, Cliente: i.client || "", Lote: i.lot || "",
              Referencia: i.reference || "", Estado: i.status,
            })),
          })}
          style={ghostButtonStyle}
        >
      ...
      {loading ? (
        <CenteredMessage text={loadError || "Cargando incidencias…"} />
      ) : issues.length === 0 ? (
        <EmptyState Icon={ShieldCheck} title="Sin incidencias registradas" message="Registra la primera no conformidad o defecto de calidad." onAdd={() => { setEditIssue(null); setModalOpen(true); }} addLabel="Crear primera incidencia" />
      ) : (
      ...
      {modalOpen && <IssueModal user={user} issue={editIssue} onClose={() => { setModalOpen(false); setEditIssue(null); }} />}
      {detailIssue && (
        <DetailModal
          issue={detailIssue}
          user={user}
          onClose={() => setDetailIssueId(null)}
          onEdit={() => { setEditIssue(detailIssue); setDetailIssueId(null); setModalOpen(true); }}
        />
      )}
      {confirmDelete && (
        <ConfirmDialog
...
function IssueModal({ user, issue, onClose }) {
  const [form, setForm] = useState(issue ? { ...emptyForm, ...issue } : emptyForm);
  ...
  async function submit(e) {
    ...
    try {
      if (issue) {
        const { id, photos, ...rest } = form;
        await updateDoc(doc(db, "quality_issues", issue.id), rest);
        logActivity(user.email, "Calidad", "Editada", form.title);
      } else {
    ...
    if (files.length) {
      ...
      try {
        for (let i = 0; i < files.length; i++) {
          ...
          const result = await withTimeout(uploadToCloudinary(blob, `quality/${docId}`), 20000, "La subida de la foto tardó demasiado.");
          uploaded.push(result);
        }
        setUploadStatus("Guardando enlaces de las fotos…");
        await updateDoc(doc(db, "quality_issues", docId), { photos: uploaded });
      } catch (err) {
        setSaving(false);
        setUploadStatus("");
        if (uploaded.length > (issue?.photos || []).length) {
          await updateDoc(doc(db, "quality_issues", docId), { photos: uploaded }).catch(() => {});
        }
        setError("La incidencia se guardó, pero no todas las fotos se pudieron subir a Cloudinary (las que sí subieron se guardaron).");
        return;
      }
    }
    setSaving(false);
  ...
  return (
    <ModalShell onClose={onClose} title={issue ? "Editar incidencia" : "Nueva incidencia de calidad"}>
      <form onSubmit={submit}>
        ...
        <Field label="Fotos">
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
            {(issue?.photos || []).map((p, i) => <img key={`ex-${i}`} src={p.url} style={{ width: 60, height: 60, objectFit: "cover" }} />)}
        ...
        <button type="submit" disabled={saving} style={{ ...primaryButtonStyle, width: "100%", justifyContent: "center", marginTop: 8 }}>
          {uploadStatus || (saving ? "Guardando…" : issue ? "Guardar cambios" : "Guardar incidencia")}
        </button>
...
function DetailModal({ issue, user, onClose, onEdit }) {
  const sev = severityMeta(issue.severity);
  ...
  async function share() {
    ...
    const result = await shareText(issue.title, lines);
  ...
  async function handleSign(blob) {
    setSigning(true);
    try {
      const result = await withTimeout(uploadToCloudinary(blob, `signatures/quality/${issue.id}`), 20000, "La firma tardó demasiado en subirse.");
      await updateDoc(doc(db, "quality_issues", issue.id), {
        status: "cerrada",
        signature: { url: result.url, signedBy: user.email, signedAt: new Date().toISOString() },
      });
      logActivity(user.email, "Calidad", "Firmada y cerrada", issue.title);
  ...
  return (
    <ModalShell onClose={onClose} title={TYPES.find((t) => t.value === issue.type)?.label}>
      ...
      <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${COLORS.line}` }}>
        {issue.signature ? (
          <div>
            ...
            <img src={issue.signature.url} alt="Firma" style={{ height: 60, background: "#fff", border: `1px solid ${COLORS.line}` }} />
            <p style={{ fontSize: 12, color: COLORS.textMuted, margin: "4px 0 0" }}>{issue.signature.signedBy} · {new Date(issue.signature.signedAt).toLocaleString("es-ES")}</p>
          </div>
        ) : (
      ...
      {showSignature && (
        <SignaturePad
          title={`Cerrar incidencia: ${issue.title}`}
