...
function MainShell({ user, role, onChangeRole }) {
  ...
  useEffect(() => {
    if (!allowedTabs.includes(tab)) setTab(visibleTabs[0]?.value || "resumen");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [role]);

  useEffect(() => {
    checkAndNotify(allowedTabs);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
      ...
      <header style={{ background: COLORS.dark }}>
        ...
        <div style={{ padding: "18px 20px", maxWidth: 1400, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <p style={{ color: "#B9B6AC", fontSize: 12, margin: 0 }}>{user.displayName ? `${user.displayName} · ` : ""}{user.email} · {roleLabel(role)}</p>
          </div>
