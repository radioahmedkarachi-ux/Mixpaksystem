...
// Guarda un archivo en el dispositivo y abre el selector nativo para
// compartirlo/guardarlo (en el navegador de escritorio, en cambio, lo
// descarga directamente — ahí sí funciona el método clásico).
async function saveAndShareFile(filename, data, mimeType, encoding) {
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      console.error("No se pudo guardar/compartir el archivo", err);
      alert(`No se pudo guardar "${filename}". Revisa el espacio disponible e inténtalo de nuevo.`);
    }
    return;
  }
  // Navegador de escritorio (útil solo si pruebas la app con "npm run dev")
...
export async function exportToCsv(filename, rows) {
  ...
  await saveAndShareFile(name, csv, "text/csv;charset=utf-8;", Encoding.UTF8);
}

// Genera un informe en PDF con cabecera de Mixpak System, una tabla y pie de
// página con la fecha y quién lo generó. `rows` es un array de objetos (como
// en exportToCsv); las columnas se sacan de las claves del primer objeto.
export async function exportToPdf({ filename, title, subtitle, userEmail, rows }) {
  if (!rows || !rows.length) return;
  try {
    const columns = Object.keys(rows[0]);
    const body = rows.map((r) => columns.map((c) => (r[c] === null || r[c] === undefined ? "" : String(r[c]))));

    const docPdf = new jsPDF({ orientation: columns.length > 6 ? "landscape" : "portrait" });
    const pageWidth = docPdf.internal.pageSize.getWidth();

    docPdf.setFillColor(242, 169, 0);
    docPdf.rect(0, 0, pageWidth, 4, "F");

    docPdf.setFontSize(16);
    docPdf.setTextColor(34, 38, 42);
    docPdf.text("Mixpak System", 14, 16);

    docPdf.setFontSize(12);
    docPdf.text(title, 14, 24);

    docPdf.setFontSize(9);
    docPdf.setTextColor(91, 88, 80);
    const dateStr = new Date().toLocaleString("es-ES", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" });
    docPdf.text(`${subtitle ? subtitle + " · " : ""}Generado: ${dateStr}${userEmail ? " · " + userEmail : ""}`, 14, 30);
    ...
    autoTable(docPdf, {
      ...
      headStyles: { fillColor: [34, 38, 42], textColor: [255, 255, 255] },
      alternateRowStyles: { fillColor: [231, 228, 220] },
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      docPdf.text(`Página ${i} de ${pageCount}`, pageWidth - 30, docPdf.internal.pageSize.getHeight() - 6);
    }

    const name = filename.endsWith(".pdf") ? filename : `${filename}.pdf`;
    if (Capacitor.isNativePlatform()) {
      const base64 = docPdf.output("datauristring").split(",")[1];
      await saveAndShareFile(name, base64, "application/pdf");
    } else {
      docPdf.save(name);
    }
  } catch (err) {
    console.error("No se pudo generar el PDF", err);
    alert("No se pudo generar el PDF. Prueba a exportar en CSV mientras lo revisamos.");
  }
}
